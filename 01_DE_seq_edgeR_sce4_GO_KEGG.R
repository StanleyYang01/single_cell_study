...
#sce5: add comparisons of all strains vs WSB

#sce4_GO_KEGG: add GO_KEGG pathway analysis just for comparions against B6
...
source("../../01_function/my_edgeR.R")

go <- goana(res, geneid = res$genes$Entrezid, species="Mm")

tr_go <- topGO(go, number = Inf) %>% filter(P.Up<1E-5|P.Down<1E-5) %>%  arrange(P.Up|P.Down, Ont)

keg <- kegga(res, geneid = res$genes$Entrezid, species="Mm")
