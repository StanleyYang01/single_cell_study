...
# df_subset$mark_Strain %>% table() # check the number of significant genes

# plot gene of strain difference
ggplot(df_subset, aes(x=logFC, y=logFDR))+
  geom_point(alpha=0.5, size=0.5)+
  geom_point(data = df_subset %>% filter(mark_Strain=="Sig"), size=1, color=strain_colors[[strains[2]]])+
...
ggplot(df_subset, aes(x=logFC, y=logFDR))+
  geom_point(alpha=0.5, size=0.5)+
  geom_point(data = df_subset %>% filter(mark_GWAS=="Sig"), size=1, color=strain_colors[[strains[2]]])+
